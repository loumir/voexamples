 ...
 *
 */
public class SearchProtocol extends GetProtocol {

	public SearchProtocol(HttpServletRequest request, ServletContext servletContext) {
	...
	public String getNiceContent() throws Exception{
		...
		if( fls.exists() ) {
			...
			for( String f: sef ) {
				File fl = new File(fls.getAbsolutePath() + "/" + f);
				if( fl.isDirectory()){
					System.out.println("@@@@@@@@@@@ " + params.useCase);
					ExampleModel em = new ExampleModel(myfile, new Request(params.protocol, f));
					if( em.match(params.useCase) ) {
						retour += "<li>" + XhtmlBuilder.getExampleAnchor(myfile, new Request(params.protocol, f)) + "</li>";
					}
				}
			}
